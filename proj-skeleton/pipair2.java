...
public class pipair {
    ...
    public static class functionNode{
        ...
        List<String> functionList = new ArrayList<>();   // dedupe is done before setting the list
    ...
    public static void main(String argv[]){
        if(argv.length == 2){
            T_SUPPORT = Integer.parseInt(argv[0]);
            T_CONFIDENCE = Integer.parseInt(argv[1]);
        }
        ...
        for(SimpleEntry pair : high_conf_pair){
            ...
            if(val_func_caller == null){
                ...
                for (String aFunc_caller : func_caller) {
                    if(val_func_caller.size() >= T_SUPPORT) {
    ...
    public static void processNode(Scanner scanner, String caller_func, int expansion_level){
        ...
        functionNode caller = map.get(caller_func);
        caller.setFunctionList(cs_list);

        // for part(c) perform expansion
        tmp_set = expand(tmp_set, expansion_level);
        cs_list = new ArrayList<>(tmp_set);
    ...
    public static Set<String> expand(Set<String> cs_set, int expansion_level){
        if(expansion_level == 0){
            return cs_set;
        }
        Set<String> tmp_set = new HashSet<>(cs_set);
        for(String call_site : cs_set){
            functionNode fn = map.get(call_site);
            List<String> fn_function_list = fn.getFunctionList();
            tmp_set.addAll(fn_function_list);
        }
        expansion_level--;
        return expand(tmp_set, expansion_level);
    }


    public static void parseCallGraph(int expansion_level){
        ...
        try {
            ...
            while(scanner.hasNext()){
                ...
                if(cur_line.startsWith("Call graph node for function:")){
                    ...
                    processNode(scanner, functionName, expansion_level);
